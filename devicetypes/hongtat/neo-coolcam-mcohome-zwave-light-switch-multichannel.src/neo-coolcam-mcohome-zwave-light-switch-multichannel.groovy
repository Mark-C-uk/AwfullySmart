...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd) {
    log.info "BasicReport() called - ${cmd.inspect()}"
    createEvent(name:"switch", value: cmd.value ? "on" : "off")
}
def zwaveEvent(physicalgraph.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) {
    log.info "SwitchBinaryReport() called - ${cmd.inspect()}"
    createEvent(name:"switch", value: cmd.value ? "on" : "off")
}
        ...
        switch(cmd.commandClass) {
            ...
            case 37:
               ...
               if (cmd.sourceEndPoint == 1) {
                    ...
                    log.info "REFRESH '$device' - '${map.value}' ---- PARENT '${map}'"
                } else {
                    ...
                    if (childDevice) {
                        ...
                        log.info "REFRESH '$childDevice' - '${map.value}' ---- Child '${map}' parent='$device'"
                    }
                }
            break
        }
...
void onOffCmd(dni, value) {
    def channels = getDataValue("endpoints").toInteger()
    if (channels > 0) {
        log.info("onOffCmd called - ${channelNumber(dni)} ${value}")
        def cmds = []
...
def refresh() {
    ...
    //log.debug "REFRESH '$device' called"
    def channels = getDataValue("endpoints").toInteger()
    if (channels > 0) {
...
def initialize() {
    ...
    if (!ccIds.find( {it == 0x60 }) ) {
        log.warn "initialize() - Device does not support MULTI_CHANNEL command classes"
...
private void createChildDevices() {
    state.oldLabel = device.label
    def channels = getDataValue("endpoints")?.toInteger()
    log.info "createChildDevices() called - Channels: ${channels}"
    if (channels > 0) {
