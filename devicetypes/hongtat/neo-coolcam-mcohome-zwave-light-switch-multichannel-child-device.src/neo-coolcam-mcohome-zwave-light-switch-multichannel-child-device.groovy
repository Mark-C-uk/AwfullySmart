...
metadata {
    definition (name: "NEO Coolcam/MCOHome Z-Wave Light Switch Multichannel Child Device", namespace: "hongtat", author: "HongTat Tan") {
        capability "Switch"
        capability "Actuator"
        capability "Sensor"
        capability "Refresh"
...
void on() {
	log.info "ON - $device (Child device)"
    parent.onOffCmd(device.deviceNetworkId, 0xFF)
}

void off() {
	log.info "OFF - $device (Child device)"
    parent.onOffCmd(device.deviceNetworkId, 0)
}

void refresh() {
	log.debug "REFRESH - $device (Child device)"
    parent.refresh()
}

void configure() {
	log.info "CONFIGURE - $device (Child device)"
    parent.configure()
}
